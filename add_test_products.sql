-- Add Test Products to PostgreSQL Database
-- Run this with: psql -U postgres -d mema_sports -f add_test_products.sql

-- Clear existing data (optional - remove if you want to keep existing data)
-- TRUNCATE TABLE product_sizes, products CASCADE;
...
-- Verify the data
SELECT 'Products added:' as message, COUNT(*) as count FROM products;
SELECT 'Product sizes added:' as message, COUNT(*) as count FROM product_sizes;

-- Show sample products
SELECT p.id, p.name, p.category, p.is_active, p.is_new, p.is_bestseller, 
       COUNT(ps.id) as size_count
FROM products p
LEFT JOIN product_sizes ps ON p.id = ps.product_id
GROUP BY p.id, p.name, p.category, p.is_active, p.is_new, p.is_bestseller
